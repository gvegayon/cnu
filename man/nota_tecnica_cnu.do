...
Adem\'as de las tasas mencionadas y las Tablas de Mortalidad, la tasa que se utiliza
...
del sistema es publicada mes a mes en el Centro de Estad\'isticas de la SP.\footnote{
...
Considerando que el c\'alculo del $CNU$ depende de los $t$ a\~nos transcurridos desde el
...
En el caso del c\'alculo utilizando un vector de tasas, es importante considerar
...
\ref{sec:ej:cnu_afili} donde se desarrolla paso a paso el caso completo del $CNU$
para una afiliada de 60 a\~nos.
...
El valor de $i_t$ depende de si se est\'a usando una tasa \'unica o un vector de tasas.
En el caso de una tasa \'unica, $i_t$ es una constante; en el caso de un vector de tasas,
$i_t$ corresponde al elemento $t$-\'esimo del vector de tasas utilizado para el
c\'alculo de la pensi\'on. Para los retiros programados calculados a partir de enero
de 2014 $i_t$ corresponde a la tasa \'unica informada por la Superintendencia de
Pensiones.
...
de la variable {\tt fsiniestro} [fecha del siniestro]),
...
\subsection{$CNU$ para Retiro Programado con Vector de Tasas (pensi\'on de vejez)\label{sec:ej:cnu_afili}}

Los par\'ametros utilizados:
\begin{itemize}
\item afiliado mujer ({\tt cotm(1)})
\item retiro programado con vector del a\~no 2013 ({\tt agnovector(2013)})
\item 67 a\~nos de edad ({\tt 67})
...
texdoc stlog
cnu_afili 67, cotm(1) pasos agnoactual(2013) agnovector(2013) tabla(rv2009)
texdoc stlog close

/*tex
\subsection{$CNU$ para Retiro Programado con Tasa \'Unica (pensi\'on de vejez)\label{sec:ej:cnu_afiliRP}}

Los par\'ametros utilizados:
\begin{itemize}
\item afiliado mujer ({\tt cotm(1)})
\item retiro programado con tasa de 3.66\% ({\tt rp(0.0366)})
\item 67 a\~nos de edad ({\tt 67})
\item calculado el 2013 ({\tt agnoactual(2013)})
\item Utilizando la tabla RV 2009 ({\tt tabla(rv2009)})
\end{itemize}
...
texdoc stlog
cnu_afili 67, cotm(1) pasos agnoactual(2013) rp(0.0366) tabla(rv2009)
texdoc stlog close

/*tex
\subsection{$CNU$ para Renta Vitalicia (pensi\'on de vejez)\label{sec:ej:cnu_afiliRV}}

Los par\'ametros utilizados:
\begin{itemize}
\item afiliado hombre (Se puede omitir la opci\'on {\tt cotm(0)})
\item renta vitalicia con tasa de 3.2\% ({\tt rv(0.032)})
\item 67 a\~nos de edad ({\tt 67})
\item calculado el 2013 ({\tt agnoactual(2013)})
\item Utilizando la tabla RV 2009 ({\tt tabla(rv2009)})
\end{itemize}
...
texdoc stlog
cnu_afili 67, pasos agnoactual(2013) rv(0.032) tabla(rv2009)
texdoc stlog close
